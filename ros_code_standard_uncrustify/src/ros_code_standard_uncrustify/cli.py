...
def get_executable() -> str:
    bundled = os.path.join(os.path.dirname(__file__), 'bin', 'uncrustify')
    if os.path.isfile(bundled) and os.access(bundled, os.X_OK):
        return bundled
    found = shutil.which('uncrustify')
    if found:
        return found
    return bundled


def run(argv: list[str] | None = None) -> Literal[0, 1]:
...
def main(argv: list[str] | None = None) -> Literal[0, 1]:
    ...
    uncrustify_bin = get_executable()
    if not uncrustify_bin or not os.path.exists(uncrustify_bin):
    ...
    parser = argparse.ArgumentParser(
        description='Check and format code style using uncrustify.',
    ...
    parser.add_argument(
        ...
        dest='reformat',
        action='store_false',
        help='Do not reformat the files in place (check only)')
    ...
    if not args.xunit_file and os.environ.get('MANYLINT_JUNIT_DIR'):
        args.xunit_file = os.path.join(
            os.environ['MANYLINT_JUNIT_DIR'], 'uncrustify.xunit.xml'
        )
    ...
    try:
        ...
        for filename, modified_filename in sorted(zip(all_input_files, all_output_files)):
            ...
            if args.reformat and diff_lines:
